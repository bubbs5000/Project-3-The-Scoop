...
function createComment(url, request){
  ...
  if(requestComment && requestComment.hasOwnProperty('comment') &&
  (requestComment.comment.body) && database.articles[requestComment.comment.articleId]
  && database.users[requestComment.comment.username]){
    ...
    reponse.body = {comment: newComment};
    reponse.status = 201;
  } else {
    reponse.status = 400;
  }
...
function deleteComment(url,request){
  ...
  if (!id || !savedComment){
    ...
    const deleteCommentId = database.articles[savedComment.articleId];
    const articleCommentId = deleteCommentId.commentIds;
    const commentUsername = savedComment.username;
    const commentUsernameId = database.users[commentUsername].commentIds;

    commentUsernameId.splice(commentUsernameId,1);
    articleCommentId.splice(deleteCommentId, 1);
